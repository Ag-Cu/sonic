...
class Expression:
    ...
    def _int(self, ch: str) -> Token:
        while not self._eof and self._hex(ch):
            ...
            elif ch[0] == '0' and len(ch) > 1:
                return Token.num(int(ch, 8))
            else:
                return Token.num(int(ch))

    ### MODIFIED: Allow '.' inside identifiers ###
    def _name(self, ch: str) -> Token:
        while not self._eof and (self._ch.isalnum() or self._ch in '_.'):
            ch += self._rch()
        else:
            return Token.name(ch)

    ### MODIFIED: Allow identifiers to start with '.' ###
    ...
    __binary__ = {
        ...
        '-'  : lambda a, b: a - b,
        '*'  : lambda a, b: a * b,
        '/'  : lambda a, b: a // b, # Use integer division
        '%'  : lambda a, b: a % b,
        '&'  : lambda a, b: a & b,
        '^'  : lambda a, b: a ^ b,
        '|'  : lambda a, b: a | b,
