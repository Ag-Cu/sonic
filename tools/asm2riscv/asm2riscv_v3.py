...
class CodeSection:
    ...
    def _trace_instructions(self, bb: BasicBlock, pcsp: Optional[Pcsp]) -> Tuple[int, bool]:
        ...
        log.debug(f"  正在分析 '{bb.name}' 中的指令 ({len(bb.body)} 条)...")
        
        for ins in bb.body:
            if isinstance(ins, LabelInstr):
                log.debug(f"    -> Encountered [LABEL]: {ins.instr}")
                continue 

            diff = 0
            if isinstance(ins, RVInstr):
                instr_obj = ins.instr
                log.debug(f"    -> Analyzing [INSTR]: {instr_obj.asm_code}")

                if instr_obj.is_return:
                    close = True
                
                if "sp" in instr_obj.asm_code:
                    parts = re.split(r'[\s,]+', instr_obj.asm_code)
                    if len(parts) >= 4 and parts[0].lower() == "addi" and parts[1] == "sp" and parts[2] == "sp":
                        log.debug(f"       ^-- Found potential SP-modifying instruction. Parts: {parts}")
                        try:
                            imm = int(parts[3])
                            diff = -imm
                        except (ValueError, IndexError):
                            log.warning(f"       ^-- Could not parse immediate from addi instruction: {instr_obj.asm_code}")
            else:
                 log.debug(f"    -> Analyzing [OTHER]: {ins.instr}")


            if diff != 0:
                log.debug(f"       ^-- SP change detected! Diff: {diff}. Block-local SP becomes: {cursp + diff}")
                cursp += diff
                if cursp > maxsp:
                    maxsp = cursp
                    log.debug(f"       ^-- New max SP for this block: {maxsp}")
    ...
    def get(self, key: str) -> Optional[int]:
        ...
        if clean_key not in self.labels:
            raise SyntaxError(f"unresolved reference to name: {clean_key}")
        return self._find_label(clean_key, itertools.repeat(0, len(self.blocks)))

    def has(self, key: str) -> bool:
        return key.lstrip('.') in self.labels
    ...
    def label(self, name: str):
        ...
        log.debug(f"Defining label: '{clean_name}' (original: '{name}')")
        if clean_name not in self.labels or self.labels[clean_name].weak:
            self._decl(clean_name, self._make(clean_name))
        else:
            raise SyntaxError("duplicated label: " + clean_name)
...
class Assembler:
    ...
    def __init__(self, pkg_name: str):
        ...
        self.functions: Dict[str, CodeSection] = {}
        self.current_function: Optional[str] = None
        self.code: Optional[CodeSection] = None
    ...
    def _get(self, v: str) -> int:
        ...
        if v not in self.vals:
            # --- BUG FIX ---
            # Ensure we use the same cleaned key to access labels.
            return self.code.get(v.lstrip('.'))
        ...
        for symbol, data_info in self.global_data.items():
            ...
            for dtype, val in buffer:
                if dtype == '.byte':
                    ...
                    byte_stream.extend(val.to_bytes(2, 'little', signed=True))
                elif dtype in ('.word', '.long', '.int'):
                    byte_stream.extend(val.to_bytes(4, 'little', signed=True))
                elif dtype == '.quad':
                    byte_stream.extend(val.to_bytes(8, 'little', signed=True))
    ...
    def _cmd_size(self, args: List[str]):
        if len(args) == 2:
            ...
            try:
                # --- BUG FIX ---
                # Ensure we use cleaned label names for calculation.
                if '-' in size_expr:
                    end, start = size_expr.split('-')
                    size = self.code.get(end.strip()) - self.code.get(start.strip())
                else:
    ...
    def _commands(self) -> dict:
        return {
            ".set": self._cmd_set, ".byte": self._cmd_byte,
            ".word": self._cmd_word, ".hword": self._cmd_word, ".short": self._cmd_word,
            ".int": self._cmd_long, ".long": self._cmd_long, ".quad": self._cmd_quad,
            ".ascii": self._cmd_ascii, ".asciz": self._cmd_asciz,
            ".space": self._cmd_space, ".zero": self._cmd_zero,
            ".p2align": self._cmd_nop, ".align": self._cmd_nop, ".globl": self._cmd_nop,
            ".text": self._cmd_nop, ".file": self._cmd_nop, ".type": self._cmd_nop,
            ".size": self._cmd_size, # Keep .size to calculate symbol sizes
            ".section": self._cmd_nop, ".attribute": self._cmd_nop,
            ".ident": self._cmd_nop, ".addrsig": self._cmd_nop,
        }
    ...
    def _parse(self, src: List[str]):
        ...
        for i, line in enumerate(src):
            ...
            if line.lower().startswith(".type"):
                ...
                if len(parts) == 3 and parts[0].lower() == '.type' and '@function' in parts[2]:
                    symbol = parts[1]
                    if self.current_function != symbol:
            ...
            # --- BUG FIX: Unified label name handling ---
            if line.endswith(":"):
                label_name = line[:-1].lstrip('.') # Always use cleaned name
                if self.current_section == 'text' and self.code:
                    self.code.label(label_name)
                else:
                    self._flush_data_buffer()
                    self.current_data_symbol = label_name # Use cleaned name for data symbols too
            ...
            if line.startswith("."):
                ...
                if cmd.cmd.lower() == '.text':
                    ...
                    self._cmd_section(cmd.args)
                    continue

                if func := self._commands.get(cmd.cmd.lower()): # Use .lower() for robustness
                    func(cmd.args)
                else:
    ...
    def _declare_body(self, asm_name: str, code_section: CodeSection):
        ...
        if asm_name in self.entry_points:
            ...
            self.out.append(f"TEXT {go_addr_getter_name}, NOSPLIT, $0-8")
...
def main():
    ...
    try:
        with open(subr_file, "w", encoding="utf-8") as fp:
            ...
            if OUTPUT_RAW:
                ...
                for name in asm.subr:
                    print(f"func __{name}_entry__() uintptr", file=fp)

                print("\nvar (", file=fp)
                mlen = max((len(s) for s in asm.subr), default=0)
                for name, entry in asm.subr.items():
                    symbol_to_call = f"__{name}_entry__"
                    print(
        ...
        log.error(f"Failed to write subroutine file: {e}", exc_info=True)
        sys.exit(1)
