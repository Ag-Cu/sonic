...
class Translator:
    ...
    def _process_instruction(self, pc: int, ins: Instruction, *, dry_run: bool) -> int:
        ...
        if lui_match:
            ...
            # --- FIX: 使用别名查找正确的符号名 ---
            # 别名表中的键已经将点替换为下划线了，所以需要先转换再查找
            normalized_symbol = clean_symbol.replace('.', '_')
            go_symbol = self.aliases.get(normalized_symbol, normalized_symbol)
            
            self.pending_lui[reg] = symbol
            if not dry_run:
                go_reg = self._translate_reg(reg)
                logging.info(f"遇到 LUI: {reg} -> {go_reg} <- hi({symbol}). 生成 MOV 地址指令 (目标: {go_symbol})。")
        ...
        if addi_match:
            ...
            if self.pending_lui.get(reg) == symbol:
                ...
                # --- MODIFICATION: 返回4以保持PC对齐 ---
                # 即使这条指令被跳过，它在原始代码中也占用了4个字节。
                # 返回4可以确保后续指令的PC地址计算正确，避免潜在的对齐问题。
                return 4
        ...
        if load_match:
            ...
            # --- FIX: 同样修正这里的符号处理逻辑 ---
            alias_lookup_key = symbol.lstrip('.').replace('.', '_')
            go_symbol_base = self.aliases.get(alias_lookup_key, alias_lookup_key)
            ...
            if self.pending_lui.get(base_reg) == symbol:
                ...
                if not dry_run:
                    ...
                    # Go汇编中，从绝对地址加载需要先将地址放入寄存器，然后解引用。
                    # 我们的LUI优化已经将地址放入了base_reg，所以这里直接解引用即可。
    ...
    # --- MODIFIED: 方法签名增加 proto_base_name ---
    def translate_c_to_binary(self, src: str, proto: Prototypes, *, name: str, proto_base_name: str):
        ...
        for sec_name, sec_ins in self.sections.items():
            ...
            for v in sec_ins:
                if isinstance(v, Label):
                    ...
                    original_label_name = v.name
                    safe_proto_base = re.sub(r'[^a-zA-Z0-9_]', '_', proto_base_name)
                    new_label_name = f"{safe_proto_base}_{original_label_name}"
                    
                    if new_label_name in self.labels: raise SyntaxError(f"Duplicate label after aliasing: {new_label_name}")
                    
                    self.labels[new_label_name] = data_pc
                    self.aliases[original_label_name] = new_label_name
                    current_label = new_label_name
                    label_start_pc = data_pc
                    logging.debug(f"Pass 1: 数据标签 '{original_label_name}' -> '{new_label_name}' 位于 offset={data_pc} (0x{data_pc:x})")
                    # --- END MODIFICATION ---
        ...
        for sec_name, sec_ins in self.sections.items():
            ...
            for v in sec_ins:
                if isinstance(v, Label):
                    # --- MODIFIED: 使用别名生成 GLOBL ---
                    original_label_name = v.name
                    aliased_label_name = self.aliases.get(original_label_name, original_label_name)
                    current_base_label = aliased_label_name
                    # --- END MODIFICATION ---
...
def main():
    ...
    out_file_base, _ = os.path.splitext(proto_file_path)
    out_file = out_file_base + '.s'
    
    proto_base_name = os.path.basename(out_file_base)
    ...
    asm.translate_c_to_binary(
        src='\n'.join(src), 
        proto=proto, 
        name=c_code_block_name,
